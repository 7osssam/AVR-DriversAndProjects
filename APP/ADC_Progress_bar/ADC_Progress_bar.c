...
void Progress_bar(void)
{
	...
	ADC_ConfigType config = {ADC_AVCC, ADC_PRESCALER_128};
	ADC_init(&config);
	...
	while (1)
	{
		...
		percent = (g_ADC_result / (double)ADC_MAXIMUM_VALUE) * 100.0;
		volt = (g_ADC_result / (double)ADC_MAXIMUM_VALUE) * Vref;
